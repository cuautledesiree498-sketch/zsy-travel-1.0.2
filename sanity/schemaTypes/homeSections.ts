...
const enabledField = { name: 'enabled', title: '显示这个模块', type: 'boolean', initialValue: true }
...
export const sectionObjects = [
  ...
  },
  {
    ...
    preview: { select: { title: 'title', enabled: 'enabled' }, prepare(value: any) { return { title: value?.title || '首页头图区', subtitle: value?.enabled === false ? '已隐藏' : '首页最上方大图区域' } } },
  },
  {
    ...
    fields: [
      ...
      { name: 'items', title: '卖点卡片列表', type: 'array', description: '每一张卡片都能单独改图标、文字和跳转。可以拖动排序。', of: [{ type: 'iconLinkCard' }], options: { sortable: true } },
    ],
    preview: { select: { title: 'title', enabled: 'enabled', count: 'items.length' }, prepare(value: any) { return { title: value?.title || '卖点图标区', subtitle: `${value?.enabled === false ? '已隐藏 · ' : ''}${value?.count || 0} 张卡片` } } },
  },
  {
    ...
    fields: [
      ...
      { name: 'title', title: '板块标题', type: 'string' },
      { name: 'subtitle', title: '板块副标题', type: 'text', rows: 3 },
      { name: 'items', title: '目的地卡片列表', type: 'array', description: '建议给每张目的地卡片上传背景图，并设置跳转链接。可以拖动排序。', of: [{ type: 'iconLinkCard' }], options: { sortable: true } },
    ],
    preview: { select: { title: 'title', enabled: 'enabled', count: 'items.length' }, prepare(value: any) { return { title: value?.title || '目的地卡片区', subtitle: `${value?.enabled === false ? '已隐藏 · ' : ''}${value?.count || 0} 张卡片` } } },
  },
  {
    name: 'tourListSection',
    title: '旅游线路区',
    type: 'object',
    fieldsets: [{ name: 'basic', title: '1. 基本内容' }, { name: 'source', title: '2. 显示哪些线路' }, { name: 'more', title: '3. 查看更多按钮' }],
    fields: [
      ...
      { name: 'sourceMode', title: '显示方式', type: 'string', description: '推荐默认选“自动展示最新线路”。', fieldset: 'source', options: { list: [{ title: '自动展示最新线路', value: 'auto' }, { title: '手动挑选推荐线路', value: 'manual' }], layout: 'radio' }, initialValue: 'auto' },
      { name: 'maxItems', title: '显示几条线路', type: 'number', initialValue: 6, fieldset: 'source', description: '只在自动模式下生效' },
      { name: 'selectedTours', title: '手动选择线路', type: 'array', fieldset: 'source', hidden: ({ parent }: any) => parent?.sourceMode !== 'manual', of: [{ type: 'reference', to: [{ type: 'tour' }] }], options: { sortable: true } },
    ...
    preview: { select: { title: 'title', enabled: 'enabled', sourceMode: 'sourceMode', maxItems: 'maxItems', count: 'selectedTours.length' }, prepare(value: any) { const sourceText = value?.sourceMode === 'manual' ? `手动挑选 ${value?.count || 0} 条` : `自动展示 ${value?.maxItems || 6} 条`; return { title: value?.title || '旅游线路区', subtitle: `${value?.enabled === false ? '已隐藏 · ' : ''}${sourceText}` } } },
  },
  {
    name: 'articleListSection',
    title: '攻略文章区',
    type: 'object',
    fieldsets: [{ name: 'basic', title: '1. 基本内容' }, { name: 'source', title: '2. 显示哪些文章' }, { name: 'more', title: '3. 查看更多按钮' }],
    fields: [
      ...
      { name: 'title', title: '板块标题', type: 'string', fieldset: 'basic' },
      { name: 'subtitle', title: '板块副标题', type: 'text', rows: 3, fieldset: 'basic' },
      { name: 'sourceMode', title: '显示方式', type: 'string', description: '推荐默认选“自动展示最新文章”。', fieldset: 'source', options: { list: [{ title: '自动展示最新文章', value: 'auto' }, { title: '手动挑选推荐文章', value: 'manual' }], layout: 'radio' }, initialValue: 'auto' },
      { name: 'maxItems', title: '显示几篇文章', type: 'number', initialValue: 3, fieldset: 'source', description: '只在自动模式下生效' },
      { name: 'selectedArticles', title: '手动选择文章', type: 'array', fieldset: 'source', hidden: ({ parent }: any) => parent?.sourceMode !== 'manual', of: [{ type: 'reference', to: [{ type: 'article' }] }], options: { sortable: true } },
      { name: 'viewMoreText', title: '查看更多按钮文字（可选）', type: 'string', fieldset: 'more' },
      { ...linkTargetField, name: 'viewMoreTarget', title: '查看更多按钮跳转到哪里', fieldset: 'more', initialValue: 'custom' },
      { name: 'viewMoreLink', title: '查看更多自定义链接', type: 'string', fieldset: 'more', hidden: ({ parent }: any) => parent?.viewMoreTarget !== 'custom' },
      { name: 'viewMoreNewTab', title: '查看更多新窗口打开', type: 'boolean', initialValue: false, fieldset: 'more' },
    ],
    preview: { select: { title: 'title', enabled: 'enabled', sourceMode: 'sourceMode', maxItems: 'maxItems', count: 'selectedArticles.length' }, prepare(value: any) { const sourceText = value?.sourceMode === 'manual' ? `手动挑选 ${value?.count || 0} 篇` : `自动展示 ${value?.maxItems || 3} 篇`; return { title: value?.title || '攻略文章区', subtitle: `${value?.enabled === false ? '已隐藏 · ' : ''}${sourceText}` } } },
  },
  {
    ...
    preview: { select: { title: 'title', enabled: 'enabled', maxItems: 'maxItems' }, prepare(value: any) { return { title: value?.title || '首页 FAQ 区', subtitle: `${value?.enabled === false ? '已隐藏 · ' : ''}显示 ${value?.maxItems || 4} 条 FAQ` } } },
  },
  {
    ...
    fields: [
      ...
      { name: 'title', title: '板块标题', type: 'string' },
      { name: 'subtitle', title: '板块副标题', type: 'text', rows: 3 },
      { name: 'items', title: '评价列表', type: 'array', options: { sortable: true }, of: [{ type: 'object', name: 'testimonialItem', fields: [{ name: 'name', title: '姓名', type: 'string' }, { name: 'country', title: '国家 / 地区', type: 'string' }, { name: 'quote', title: '评价内容', type: 'text', rows: 4 }, { name: 'rating', title: '星级（1-5）', type: 'number', initialValue: 5 }, { name: 'avatar', title: '头像（可选）', type: 'image', options: { hotspot: true } }], preview: { select: { title: 'name', subtitle: 'country' } } }] },
    ],
    preview: { select: { title: 'title', enabled: 'enabled', count: 'items.length' }, prepare(value: any) { return { title: value?.title || '用户评价区', subtitle: `${value?.enabled === false ? '已隐藏 · ' : ''}${value?.count || 0} 条评价` } } },
  },
  {
    name: 'statsSection',
    title: '数据统计区',
    ...
    fields: [
      ...
      { name: 'title', title: '板块标题', type: 'string' },
      { name: 'subtitle', title: '板块副标题', type: 'text', rows: 3 },
      { name: 'items', title: '统计项列表', type: 'array', options: { sortable: true }, of: [{ type: 'object', name: 'statItem', fields: [{ name: 'number', title: '数字', type: 'string' }, { name: 'label', title: '说明', type: 'string' }], preview: { select: { title: 'number', subtitle: 'label' } } }] },
    ],
    preview: { select: { title: 'title', enabled: 'enabled', count: 'items.length' }, prepare(value: any) { return { title: value?.title || '数据统计区', subtitle: `${value?.enabled === false ? '已隐藏 · ' : ''}${value?.count || 0} 个数字` } } },
  },
  {
    name: 'ctaSection',
    title: '底部行动按钮区',
    type: 'object',
    fieldsets: [{ name: 'content', title: '1. 文案' }, { name: 'media', title: '2. 图片' }, { name: 'actions', title: '3. 按钮' }],
    fields: [
    ...
    preview: { select: { title: 'title', enabled: 'enabled', primaryButtonText: 'primaryButtonText' }, prepare(value: any) { return { title: value?.title || '底部行动按钮区', subtitle: `${value?.enabled === false ? '已隐藏' : '主按钮：' + (value?.primaryButtonText || '未设置')}` } } },
  },
]
