...
export default async function DestinationDetailPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ lang?: string }> }) {
  ...
  const introLine = lang === 'zh'
    ? `如果你想把 ${displayName} 作为整条中国路线中的一站，我们会更关注它在节奏、审美和整体体验里的位置。`
    : `If you want to place ${displayName} inside a wider China itinerary, we care most about its rhythm, tone and role in the overall journey.`;
  const valueLine = lang === 'zh'
    ? '这类定制并不只是“去哪里”，而是把停留时间、移动节奏、体验强度和风格偏好一起收进去。'
    : 'Custom planning is not just about where to go — it is about timing, pace, experience intensity and the style that fits you best.';
  const consultLine = lang === 'zh'
    ? '如果你有出行日期、人数、预算或偏好的风景类型，可以直接发给我们，我们会把它整理成更完整的路线建议。'
    : 'If you already have dates, group size, budget or scenery preferences, send them over and we will shape them into a fuller route recommendation.';
        ...
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-8">
            ...
            <div className="grid gap-6 md:grid-cols-3">
              <MiniCard title={lang === 'zh' ? '适合人群' : 'Ideal For'} value={audience} />
              <MiniCard title={lang === 'zh' ? '最佳时间' : 'Best Time'} value={bestSeason} />
              <MiniCard title={lang === 'zh' ? '建议停留' : 'Suggested Stay'} value={stay} />
            </div>
          ...
          <aside className="space-y-6">
            <div className="sticky top-24 rounded-[2rem] border border-[rgba(10,27,52,0.08)] bg-[linear-gradient(180deg,#ffffff,#f7faff)] p-8 shadow-[0_24px_60px_rgba(10,27,52,0.08)]">
              <p className="text-xs uppercase tracking-[0.28em] text-[var(--color-muted)]">{lang === 'zh' ? '定制建议' : 'Custom Planning Note'}</p>
              ...
              <div className="mt-7 flex flex-col gap-3">
                <Link href={withLang('/contact', lang)} className="inline-flex items-center justify-center rounded-full bg-[var(--color-navy)] px-6 py-3 text-sm font-semibold uppercase tracking-[0.16em] text-white transition hover:bg-[var(--color-navy-soft)]">
                  {lang === 'zh' ? '开始定制咨询' : 'Start Custom Planning'}
                </Link>
