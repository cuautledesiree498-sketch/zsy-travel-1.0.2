    ...
    <main className="mx-auto max-w-7xl px-6 py-24">
      ...
      <section className="mt-14 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
        {destinations.length > 0 ? destinations.map((item: any) => {
          const title = markPlaceholder(pickLocalized(item.name, lang) || (lang === 'zh' ? '目的地名称待补充' : 'Destination name coming soon'));
          const desc = markPlaceholder(pickLocalized(item.description, lang) || (lang === 'zh' ? '目的地简介待补充' : 'Description coming soon'));
          return (
