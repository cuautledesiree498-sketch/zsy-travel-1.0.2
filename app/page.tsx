...
export default async function Home() {
  ...
  const navCtaLink = settings?.headerCtaLink || heroSection?.primaryButtonLink || '/contact';
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex justify-between items-center gap-4">
            ...
            <div className="hidden md:flex items-center space-x-8">
              {sections
                .filter((section: any) => section?.anchorId)
        ...
        <div className="relative z-10 text-center text-white px-4 max-w-4xl">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            {heroSection?.primaryButtonText && (
              <SmartLink href={heroSection.primaryButtonLink || '#tours'} newTab={heroSection.primaryButtonNewTab} className="bg-blue-600 text-white px-10 py-4 rounded-full text-xl font-semibold hover:bg-blue-700 transition shadow-2xl inline-block">
                {heroSection.primaryButtonText}
              </SmartLink>
            )}
            {heroSection?.secondaryButtonText && (
              <SmartLink href={heroSection.secondaryButtonLink || '/contact'} newTab={heroSection.secondaryButtonNewTab} className="bg-white/20 backdrop-blur-sm text-white px-10 py-4 rounded-full text-xl font-semibold hover:bg-white/30 transition border-2 border-white inline-block">
                {heroSection.secondaryButtonText}
              </SmartLink>
            )}
          </div>
        ...
        switch (section._type) {
          ...
          case 'tourListSection': {
            const list = section.sourceMode === 'manual'
              ...
              : tours.slice(0, section.maxItems || 6);
            return <TourListSection key={`${section._type}-${index}`} section={section} tours={list} />;
          }
          case 'articleListSection': {
            const list = section.sourceMode === 'manual'
              ? (section.selectedArticles || []).filter((item: any) => item?.published !== false)
              : articles.slice(0, section.maxItems || 3);
      ...
      <footer id="footer-contact" className="bg-gray-900 text-white py-16">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div>
              ...
              <div className="space-y-3">
                {(settings?.socialLinks || []).map((item: any, index: number) => (
                  <SmartLink key={index} href={item?.url} newTab={item?.newTab} className="flex items-center gap-2 text-gray-400 hover:text-white transition">
                    <span>{socialIconMap[item?.platform] || '🔗'}</span>
                    <span>{item?.label || item?.platform || 'Link'}</span>
                  </SmartLink>
                ))}
              </div>
            </div>
          ...
          <div className="border-t border-gray-800 mt-12 pt-8 text-center text-gray-500">
            <p>&copy; 2026 {settings?.siteTitle || 'Xinjiang Travel'}. All rights reserved.</p>
          </div>
        </div>
      </footer>
    </div>
...
function SmartLink({ href, newTab, className, children }: any) {
  ...
  if (isInternal) {
    return <Link href={href} className={className} target={newTab ? '_blank' : undefined} rel={newTab ? 'noopener noreferrer' : undefined}>{children}</Link>;
  }
...
function SectionHeader({ title, subtitle }: { title?: string; subtitle?: string }) {
  return (
    <div className="text-center mb-16 max-w-3xl mx-auto px-4">
      {title && <h3 className="text-4xl font-bold text-gray-800 mb-4">{title}</h3>}
...
function getDefaultSectionLabel(type: string) { const map: Record<string, string> = { heroSection: 'Home', featureIconsSection: 'Highlights', destinationCardsSection: 'Destinations', tourListSection: 'Tours', articleListSection: 'Guides', ctaSection: 'Plan Your Trip' }; return map[type] || 'Section'; }
